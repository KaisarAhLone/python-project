...
class SudokuGame:
    def __init__(self, root):
        ...
        self.root.configure(bg="#FFF5EE")  # Warm pastel background
        self.sudoku = Sudoku()
        self.entries = [[None for _ in range(9)] for _ in range(9)]
        self.original_grid = None
        self.create_widgets()
        self.color_index = 0
        self.colors = ["#1E90FF", "#87CEEB", "#B0E57C", "#FFD700", "#FF6347", "#8A2BE2", "#FF69B4"]
        self.fade_step = 0
        self.start_color_animation()

    def create_widgets(self):
        menu_frame = tk.Frame(self.root, bg="#FFF5EE")
        menu_frame.grid(row=0, column=0, columnspan=9, pady=10)

        tk.Label(menu_frame, text="Select Difficulty:", bg="#FFF5EE", font=("Verdana", 12, "bold")).pack(side=tk.LEFT, padx=10)
        ...
        tk.Button(menu_frame, text="Validate", command=self.validate_progress, bg="#90EE90", fg="#000",
                  font=("Verdana", 10), activebackground="#32CD32").pack(side=tk.LEFT, padx=10)
        tk.Button(menu_frame, text="New Game", command=self.new_game, bg="#FFDAB9", fg="#000",
                  font=("Verdana", 10), activebackground="#FFB347").pack(side=tk.LEFT, padx=10)
        tk.Button(menu_frame, text="Solve", command=self.solve_puzzle, bg="#FF4500", fg="white",
                  font=("Verdana", 10), activebackground="#FF6347").pack(side=tk.LEFT, padx=10)
        ...
        for row in range(9):
            for col in range(9):
                ...
                entry.configure(bg="#FDF5E6")  # Light cream color
        ...
        for row in range(9):
            for col in range(9):
                ...
                if value != 0:
                    ...
                    entry.configure(state="disabled", fg="#2F4F4F")
                else:
        ...
        for row in range(9):
            for col in range(9):
                if player_grid[row][col] != 0 and not self.sudoku.is_valid(row, col, player_grid[row][col]):
                    self.entries[row][col].configure(bg="#FFB6C1")
                else:
                    self.entries[row][col].configure(bg="#FDF5E6")
    ...
    def clear_grid(self):
        for row in range(9):
            for col in range(9):
                entry = self.entries[row][col]
                entry.delete(0, tk.END)
                entry.configure(state="normal", bg="#FDF5E6")
